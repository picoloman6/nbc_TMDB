...
const setPageButtons = () => {
  ...
  for (let i = 1; i <= 6; i++) {
    numberButtonWrapper.innerHTML += `<span class="number-button" type="button"> ${i} </span>`;
  }
};
setPageButtons();

// 카드 지웠다가 pageNumber에 해당하는 페이지 카드 붙이는 함수

// const setPageOf = async (pageNumber) => {
//   try {
//     const url = `https://api.themoviedb.org/3/movie/top_rated?language=ko-KR&page=${pageNumber}`;
//     const res = await fetch(url, options).then((response) => response.json());
//     data = res.results;
